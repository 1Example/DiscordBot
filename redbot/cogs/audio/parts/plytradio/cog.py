...
@cog_i18n(_)
class PyLavYouTubeRadio(YouTubeRadioDashboard, DISCORD_COG_TYPE_MIXIN):
    ...
    def _ytradio_init(self) -> None:
        self._ytradio_config = Config.get_conf(
            None, identifier=208903205982044162, cog_name="PyLavYouTubeRadio"
        )
        self._ytradio_config.register_guild(
            enabled=False,
            buffer=3,
        )
    ...
    async def _ytradio_unload(self) -> None:
        self._played.clear()
        self._seeds.clear()

    async def is_enabled(self, guild_id: int) -> bool:
        if guild_id not in self._enabled_cache:
            self._enabled_cache[guild_id] = await self._ytradio_config.guild_from_id(guild_id).enabled()
        return self._enabled_cache[guild_id]

    async def get_buffer(self, guild_id: int) -> int:
        if guild_id not in self._buffer_cache:
            self._buffer_cache[guild_id] = await self._ytradio_config.guild_from_id(guild_id).buffer()
    ...
    async def _ytradio_on_pylav_track_start_event(self, event: TrackStartEvent) -> None:
    ...
    @commands.Cog.listener("on_pylav_track_skipped_event")
    async def _ytradio_on_pylav_track_skipped_event(self, event: TrackSkippedEvent) -> None:
    ...
    @commands.Cog.listener("on_pylav_track_end_event")
    async def _ytradio_on_pylav_track_end_event(self, event: TrackEndEvent) -> None:
