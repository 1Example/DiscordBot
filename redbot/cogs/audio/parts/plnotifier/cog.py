...
@cog_i18n(_)
class PyLavNotifier(NotifierDashboard, DISCORD_COG_TYPE_MIXIN):
    ...
    def _notifier_init(self) -> None:
        self._notifier_config = Config.get_conf(
            None, identifier=208903205982044161, cog_name="PyLavNotifier"
        )
        ...
        self._notifier_config.register_global(notify_channel_id=None)
        self._notifier_config.register_guild(
    ...
    async def _notifier_initialize(self, *args, **kwargs) -> None:
        for guild_id, guild_data in (await self._notifier_config.all_guilds()).items():
    ...
    @staticmethod
    def _track_of(event):
        """The track an event is about, whatever the event calls it.

        Events carry a single `track`, a list of `tracks`, or nothing at all
        (in which case whatever the player is on right now is the best guess).
        """
        track = getattr(event, "track", None)
        if track is not None:
            return track
        tracks = getattr(event, "tracks", None)
        if tracks:
            try:
                return tracks[0]
    ...
    async def _notify(
        ...
        *,
        ...
        if event is not None and self._is_bot_action(event, member):
            try:
                members_only = await self._notifier_config.guild(channel.guild).members_only()
        ...
        if kind:
            try:
                colour_coded = await self._notifier_config.guild(channel.guild).colour_coded()
            except Exception:  # noqa: BLE001
                colour_coded = True
            if colour_coded:
    ...
    async def send_embed_batch(
        ...
        try:
            delete_after = await self._notifier_config.guild(channel.guild).auto_delete_after()
    ...
    async def on_pylav_track_exception_event(self, event: TrackExceptionEvent) -> None:
        ...
        if channel is None:
            return
        data = await self._notifier_config.guild(guild=event.player.guild).get_raw(
    ...
    async def _node_channels(self, key: str) -> list:
        ...
        channels = []
        try:
            all_guilds = await self._notifier_config.all_guilds()
        except Exception:  # noqa: BLE001
