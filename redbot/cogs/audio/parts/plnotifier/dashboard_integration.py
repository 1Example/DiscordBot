...
class NotifierDashboard:
    ...
    async def dashboard_plnotifier_page(
        ...
        settings = await self._notifier_config.guild(guild).all()
        grouped, seen = [], set()
    ...
    async def _pln_handle_post(self, guild: discord.Guild, kwargs: dict) -> list[dict]:
        ...
        action = field("action")
        conf = self._notifier_config.guild(guild)
        ...
        try:
            ...
            if action == "reset_basics":
                changed = 0
                async with self._notifier_config.guild(guild).all() as stored:
