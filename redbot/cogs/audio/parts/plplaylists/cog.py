...
class PyLavPlaylists(
    ...
    def _playlists_init(self) -> None:
        self._playlists_config = Config.get_conf(
            None, identifier=208903205982044161, cog_name="PyLavPlaylists"
        )

    async def _playlists_delete_user_data(
        self,
        *,
    ...
    async def slash_playlist_mix(
        ...
        await self._playlists_process_play_message(
            context, single_track, total_tracks_enqueue, [query]
        )

    async def _playlists_process_play_message(self, context, single_track, total_tracks_enqueue, queries):
        artwork = None
        file = None
