...
class PersistentControllerView(discord.ui.View):
    ...
    async def _apply_button_styling(self) -> None:
        ...
        """
        try:
            settings = await self.cog._controller_config.guild(self.guild).all()
    ...
    async def _build_now_playing_embed(self, player) -> discord.Embed:
        ...
        try:
            base_url = await self.cog._controller_config.dashboard_url()
        except Exception:  # noqa: BLE001
            base_url = ""
        if base_url:
            embed.add_field(
    ...
    async def _drop_rejected_emoji(self, exc: discord.HTTPException) -> bool:
        ...
        if key is not None:
            with contextlib.suppress(Exception):
                conf = self.cog._controller_config.guild(self.guild)
    ...
    async def _charge_for(self, interaction: DISCORD_INTERACTION_TYPE, button_name: str) -> bool:
        ...
        if await self._may_manage(interaction):
            return True
        try:
            config = self.cog._controller_config.guild(self.channel.guild)
