...
class PyLavController(
    ...
    async def command_plcontrollerset_cost(self, context: PyLavContext, action: str, amount: int) -> None:
        ...
        if amount < 0:
            ...
            return
        async with self._controller_config.guild(context.guild).dashboard_action_costs() as costs:
            if amount == 0:
    ...
    async def command_plcontrollerset_costs(self, context: PyLavContext) -> None:
        ...
        costs = await self._controller_config.guild(context.guild).dashboard_action_costs()
        if not costs:
    ...
    async def command_plcontrollerset_channel(
        ...
        await self._controller_config.guild(context.guild).channel.set(channel.id)
    ...
    async def command_plcontrollerset_acceptrequests(self, context: PyLavContext):
        ...
        if context.guild.id not in self._channel_cache or (
            ...
            return
        current = await self._controller_config.guild(context.guild).list_for_requests()
        await self._controller_config.guild(context.guild).list_for_requests.set(not current)
    ...
    async def command_plcontrollerset_acceptsearches(self, context: PyLavContext):
        ...
        if (channel_id := self._channel_cache.get(context.guild.id)) is None or channel_id not in self._view_cache:
            ...
            return
        current = await self._controller_config.guild(context.guild).list_for_searches()
        await self._controller_config.guild(context.guild).list_for_searches.set(not current)
    ...
    async def command_plcontrollerset_slowmode(self, context: PyLavContext):
        ...
        if (channel_id := self._channel_cache.get(context.guild.id)) is None or channel_id not in self._view_cache:
            ...
            return
        current = await self._controller_config.guild(context.guild).use_slow_mode()
        await self._controller_config.guild(context.guild).use_slow_mode.set(not current)
    ...
    async def command_plcontrollerset_antispam(self, context: PyLavContext):
        ...
        current = await self._controller_config.guild(context.guild).enable_antispam()
        await self._controller_config.guild(context.guild).enable_antispam.set(not current)
    ...
    async def command_plcontrollerset_greedy(self, context: PyLavContext):
        ...
        await self._controller_config.listen_to_any_message.set(self._greedy_cache)
