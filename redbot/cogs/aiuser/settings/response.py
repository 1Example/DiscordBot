...
class ResponseSettings(MixinMeta):
    ...
    async def removelist_reset(self, ctx: commands.Context):
        ...
        embed = discord.Embed(
            title="Are you sure?",
            description="This will reset this server's response filters to default.",
            color=await ctx.embed_color(),
        )
        ...
        return await confirm.edit(
            embed=discord.Embed(
                title="Response filters reset.", color=await ctx.embed_color()
            )
        )

    @response.group(
        name="weights", aliases=["logit_bias", "bias"], invoke_without_command=True
    )
    @checks.admin_or_permissions(manage_guild=True)
    async def weights(self, ctx: commands.Context):
        """
        Show weights
        """
    ...
    async def set_weight(self, ctx: commands.Context, word: str, weight: int):
        ...
        *Arguments*
        ...
        try:
            ...
            return await ctx.send(
                ":warning: Unsupported model, please use custom parameters instead."
            )
    ...
    async def remove_weight(self, ctx: commands.Context, word: str):
        ...
        *Arguments*
        ...
        if len(token) == 1:
            ...
            return await ctx.send(
                ":warning: Word is multiple tokens? Please check [p]aiuser response weights list"
            )
    ...
    @response.group(name="parameters", invoke_without_command=True)
    @checks.is_owner()
    async def custom_parameters(self, ctx: commands.Context):
        """Show custom endpoint parameters"""
    ...
    @custom_parameters.command(name="set")
    async def set_custom_parameters(self, ctx: commands.Context, *, json_block: str):
        """Set custom endpoint parameters using a JSON code block"""
        if not json_block.startswith("```"):
            return await ctx.send(
                ":warning: Please provide the parameters in a JSON code block."
            )
        ...
        if not isinstance(data, dict):
            return await ctx.send(":warning: Parameters must be a JSON object.")
        ...
        if data.get("logit_bias") and await self.config.guild(ctx.guild).weights():
            ...
            if not confirmed:
                return
            await confirm.edit(
                content="Overwritten existing weights.", embed=None, view=None
            )
            await self.config.guild(ctx.guild).weights.set(None)

        await self.config.guild(ctx.guild).parameters.set(json.dumps(data))
        return await ctx.send("Custom endpoint parameters updated.")

    @custom_parameters.command(name="clear")
    async def custom_parameters_clear(self, ctx: commands.Context):
        """Clear custom endpoint parameters"""
        await self.config.guild(ctx.guild).parameters.set(None)
        return await ctx.send("Custom endpoint parameters cleared.")
